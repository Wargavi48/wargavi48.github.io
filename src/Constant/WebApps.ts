const webAppsData = [
  {
    id: 1,
    imageLink:
      "https://pbs.twimg.com/media/HF4QDd8bEAAUCIj?format=jpg&name=orig",
    title: "Wargavi48 Discord Hub",
    description:
      "Community gateway for discussion, events, and fan coordination.",
    article:
      "Join the main community hub to connect with other members, follow updates, and participate in activities.",
    projectLink: "https://wargavi48.github.io/discord",
  },
  {
    id: 2,
    imageLink: "https://img.youtube.com/vi/fsWSYE1E8Eo/maxresdefault_live.jpg",
    title: "WGV: Dreamcatcher",
    description:
      "Fan made Visual Novel and original song project about JKT48V Generation 1.",
    article:
      'A dedicated web page for the Dreamcatcher project.\n\nWargavi48 X [Rizuka Miku](https://www.youtube.com/@RizukaMiku_Vtuber)\nDreamcatcher\n\nVerse:\nMentari pagi perlahan menyusuri\nRuang kamar ini, dengan semangat menerangi\nKubuka mata tuk sambut hari ini\nAku akan melangkah tuk hadapi dunia\n\nPre-Chorus:\nWalau terkadang hujan turun\nBasahiku dan coba patahkan semangatku\nTapi ku takkan pernah menyerah\n\nChorus:\nLarilah dan terus percaya\nCerita mengukir mimpi telah dimulai\nBerjuang satukan harapan\nSampai impian menjadi nyata, terus percaya\nDreamcatcher!\n\nVerse:\nAngin berbisik seolah memberitahu\nAku harus bergegas tuk menggapai mimpi\n\nPre-Chorus:\nWalau rintangan di depanku\nTak jadi alasanku untuk berhenti\nKarena ku takkan pernah menyerah\n\nChorus:\nTerbanglah dan terus bermimpi\nHingga bintang-bintang itu bisa tergapai\nCahaya yang penuh harapan\nPasti impian kan jadi nyata, terus percaya\nDreamcatcher!\n\nBridge:\nJalan yang penuh lika-liku\nKu tak peduli, berapa kali aku terjatuh\n\nChorus:\nLarilah dan terus percaya\nCerita mengukir mimpi telah dimulai\nBerjuang satukan harapan\nSampai impian menjadi nyata, terus percaya\nDreamcatcher!\n<iframe width="560" height="315" src="https://www.youtube.com/embed/fsWSYE1E8Eo?si=KJz6soz-rfICmo7V&amp;controls=0" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>',
    projectLink: "https://wargavi48.github.io/wgvdreamcatcher",
  },
  {
    id: 3,
    imageLink:
      "https://pbs.twimg.com/media/GWdLi6AbkAAr_27?format=png&name=orig",
    title: "Wargavi48 KTV",
    description: "Web landing and information page for the KTV project.",
    article: "Wargavi48 virtual ID Card.\n\nSource tweet: https://x.com/wargavirtual48/status/1830515339513508247",
    projectLink: "https://wargavi48.github.io/ktv/",
  },
  {
    id: 4,
    imageLink:
      "https://pbs.twimg.com/media/GWhGNIYaoAAPHMg?format=jpg&name=orig",
    title: "Bola-bola KTP",
    description: "JKT48V Generation 1 fan made suika game.",
    article:
      "Selamat pagi para warga 🔆\nAda yang baru lagi nih dari minvi..\n\nKita berpacu dalam game gabut bola-bola KTP⚽\n\nP adu score bro 😎\n\n#Piallete #IrengDoesArt #Kanartea\n\nSource tweet: https://x.com/wargavirtual48/status/1830792687689310520",
    projectLink: "https://wargavi48.github.io/bola-ktp",
  },
  {
    id: 5,
    imageLink:
      "https://pbs.twimg.com/media/GjrDNw1aMAAieB8?format=jpg&name=orig",
    title: "#ChekiChoco #KeyToParadise",
    description: "Project to Celebrate Valentine Day 2025 with JKT48V",
    article:
      "🌤️🔐Key To Paradise🔐🌤️\nHeeey para warga~\nBesok hari Valentine dan KTP bakal ada baju baru nih!\nUntuk merayakan Project Outfit 2.0 dan Valentine, Minvi udah nyiapin project seru:\n\nSiapa yang mau foto berdua sama KTP?\nAku! Aku! Aku!\nMinVi adain Cheki 2Shot Online nih, hehehe 😆\n#KeyToParadise #ChekiChoco #KanaBaru #TanaBaru #PiaBaru\n\nSource tweet: https://x.com/wargavirtual48/status/1890038267841823071",
    projectLink: "https://wargavi48.github.io/chekichoco",
  },
  {
    id: 6,
    imageLink:
      "https://pbs.twimg.com/media/G9_SQkpawAA2iAp?format=jpg&name=orig",
    title: "#BINGOJKT48V",
    description: "JKT48V fan made live stream bingo game.",
    article:
      "Yuk main bingo bareng!!\n\nKalian bisa main bareng Minvi sambil nonton stream JKT48V.\n\nYuk tunggu apalagi, mainin sekarang juga!!\n\nhttps://wargavi48.github.io/bingo/",
    projectLink: "https://wargavi48.github.io/bingo/",
  },
  {
    id: 7,
    imageLink:
      "https://pbs.twimg.com/media/HL0wGnpbQAAELre?format=png&name=orig",
    title: "Grid JKT48V",
    description: "JKT48V fan made grid game.",
    article:
      "Sambil dengerin SSB, kita mainan grid JKT48V guys\ncoba reply punya kalian gaes, ini punya minvi 😇\n\nSource tweet: https://x.com/wargavirtual48/status/2070864786020237662",
    projectLink: "https://grids.fun/t/jkt48v-7ugvj?list=jktv-gfc3r",
  },
];

export default webAppsData;
